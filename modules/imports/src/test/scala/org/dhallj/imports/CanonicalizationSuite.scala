...
class CanonicalizationSuite extends FunSuite {
  ...
  test("Paths - Trailing .") {
    assertEquals(canonicalize[IO](Local(Paths.get("/foo/."))).unsafeRunSync, Local(Paths.get("/foo")))
  }

  test("Paths - Trailing ..") {
    assertEquals(canonicalize[IO](Local(Paths.get("/foo/bar/.."))).unsafeRunSync, Local(Paths.get("/foo")))
  }
